...
const Cards = ({ sortingData, idData }) => {
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-8">
      {sortingData.map((singleCard, index) => (
        <Card key={index} singleCard={singleCard} idData={idData}></Card>
      ))}
    </section>
  );
};
